...
public final class MathTopComponent extends JPanel implements MultiViewElement {
  ...
  private void initComponents() {
    ...
    exprList = new javax.swing.JList<ExprNode>();
          ...
          do {
            ...
            do {
              ...
              if (schema instanceof Result && schema.isReady()) {
                ...
                if (schema.getRgs().length > 0) { // liste des parents
                  for (int i = 0; i < schema.getRgs().length; i++) {
                    schema.getRgs()[i] = exprNodes.get(schema.getRgs()[i] - 1).getRange();
                  }
                ...
                if (!schema.isReady()) {
                  schema.setReady(schema.getRgs()[last] > inf); // l'un au moins est plus grand
                }
                for (int i = schema.getRgs()[last]; i < oldsize; i++) { // parcours de la liste
                  schema.getRgs()[last] = i + 1;
  ...
  private javax.swing.JList<ExprNode> exprList;
